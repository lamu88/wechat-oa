...
<!-- 	<div class="wrapper b-b header"><b>投票中心</b></div>
	<ul class="nav">
	 <li class="b-b b-light open">
			<a href="<?php echo U('Vote/index',array('id'=>$_GET['id']));?>"><i class="fa fa-chevron-right pull-right m-t-xs text-xs icon-muted"></i>查看投票</a>
		</li>
	...
	<ul class="nav">
	 <li class="b-b b-light open">
			<a href="<?php echo U('Vote/index',array('mid'=>$_GET['mid']));?>" target="_self"><i class="fa fa-chevron-right pull-right m-t-xs text-xs icon-muted"></i>查看投票</a>
		</li>
                        ...
                        <div class="col-sm-8 m-b-xs">
                            ...
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu">
                                    <li>
                                        <a href="<?php echo U('Vote/add',array('mid'=>$_GET['mid'],'type'=>1,'id'=>$_GET['id']));?>" target="_self">文字投票</a>
                                       
                                    </li>
                                    <li>
                                        <a href="<?php echo U('Vote/add',array('mid'=>$_GET['mid'],'type'=>2,'id'=>$_GET['id']));?>" target="_self">图片投票</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="btn-group m-l">
                                <a href="<?php echo U('Vote/index',array('mid'=>$_GET['mid'],'status'=>0,'id'=>$_GET['id']));?>" target="_self" class="btn btn-sm btn-default "  title="全部">
                                    查看所有
                                </a>
                                <a href="<?php echo U('Vote/index',array('mid'=>$_GET['mid'],'status'=>1,'id'=>$_GET['id']));?>" target="_self" class="btn btn-sm btn-default" title="正在进行">
                                    正在进行
                                </a>
                                <a href="<?php echo U('Vote/index',array('mid'=>$_GET['mid'],'status'=>2,'id'=>$_GET['id']));?>" target="_self" class="btn btn-sm btn-default" title="未开启">
                                    未开启
                                </a>
                                <a href="<?php echo U('Vote/index',array('mid'=>$_GET['mid'],'status'=>3,'id'=>$_GET['id']));?>" target="_self" class="btn btn-sm btn-default" title="已结束">
                                    已结束
                                </a>
                            </div>
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover m-b-none entity-view">
							...
							<?php if($data): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($data["id"]); ?>">
									...
									<td>
										<?php if($data['type'] == 1): ?>单选<?php endif; ?>
										<?php if($data['type'] == 2): ?>多选<?php endif; ?>
										<?php if($data['type'] == 5): ?>评级<?php endif; ?>
										<?php if($data['type'] == 3): ?>多图对比<?php endif; ?>
										<?php if($data['type'] == 4): ?>赞成反对<?php endif; ?>
			                        </td>
									<td><?php echo ($data["nums"]); ?></td>
									<td><?php echo (date("Y-m-d H:i",$data["starttime"])); ?></td>
									<td>
									    <?php if($data['status'] == 1): ?>进行中<?php endif; ?>
			                            <?php if($data['status'] == 3): ?>已结束<?php endif; ?>
										<?php if($data['status'] == 2): ?>未开启<?php endif; ?>
									</td>
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("index.php?g=Qyapp&m=Vote&a=info&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
