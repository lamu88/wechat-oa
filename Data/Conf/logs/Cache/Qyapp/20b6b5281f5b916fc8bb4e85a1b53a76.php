...
<aside class="aside-md bg-white b-r" id="subNav">
	 	<div class="wrapper b-b header"><b>任务协作</b></div>
	<ul class="nav">
		<li class="b-b b-light open">
			<a href="<?php echo U('Task/index',array('mid'=>$_GET['mid']));?>" target="_self"><i class="fa fa-chevron-right pull-right m-t-xs text-xs icon-muted"></i>任务管理</a>
		</li>				
                    ...
                    <section class="panel panel-default ">
                        <div class="table-responsive">
                            <table class="table table-hover m-b-none entity-view" data-path="/task/detail/id/{0}">
                                ...
                                <tbody id="mytable">
								<?php if($data): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr data-id="1607" id="<?php echo ($data["id"]); ?>">
                                            <td><?php echo ($data["content"]); ?></td>
                                            <td><?php echo (date("Y-m-d H:i",$data["mktime"])); ?></td>
                                            <td><?php echo ($data["fuzeren"]); ?></td>
											<td><?php echo ($data["helper"]); ?></td>
											<td><?php echo (date("Y-m-d ",$data["end_time"])); ?></td>
                                            <td><?php if($data['order'] == '1'): ?>低<?php endif; if($data['order'] == '2'): ?>中<?php endif; if($data['order'] == '3'): ?>高<?php endif; ?></td>
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("index.php?g=Qyapp&m=Task&a=info&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
