...
<section class="hbox stretch">
    <aside class="aside-md bg-white b-r" id="subNav">
     ...
     <ul class="nav" style='overflow-y:auto; height:100%'>
            <li class="b-b b-light open"><a href="<?php echo U('Showadmin/nlist');?>" target="_self"><i class="fa fa-chevron-right pull-right m-t-xs text-xs icon-muted"></i>管理员列表</a></li>
			<li class="b-b b-light"><a href="javascript:;" onclick="addGroup()" target="_self" ><i class="fa fa-chevron-right pull-right m-t-xs text-xs icon-muted"></i>新建管理组</a></li>
                    <!-- <?php if(is_array($infos)): $i = 0; $__LIST__ = $infos;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$infos): $mod = ($i % 2 );++$i;?><li class="b-b b-light">
						   <a href="<?php echo U('Showadmin/editgroup',array('id'=>$infos['id']));?>" target="_self">
							<?php echo ($infos["name"]); ?>                
							</a>
        ...
        <header class="header bg-white b-b clearfix">
			<div class="row m-t-sm">
				<!-- <div class="col-sm-8 m-b-xs">
					...
					<a href="<?php echo U('Showadmin/nlist');?>" target="_self" class="btn btn-sm btn-default"><i class="fa fa-list-ul m-r-xs"></i>管理员列表</a>
				</div> -->
            </div>
        </header>
            ...
            <section class="panel panel-default ">
                <div class="table-responsive">
                    <table class="table table-hover m-b-none entity-view">
                        ...
                        <tbody id="mytable">
						<?php if($info): if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($info["id"]); ?>">
                            <td ><img src="<?php echo ($info["pic"]); ?>" alt="" class="face" /><?php echo ($info["name"]); ?>  </td>
                            <td ><?php echo ($info["name"]); ?>  @<?php echo ($info["uid"]); ?>  </td>
...
function addGroup(){
	$('#addGroup').load("<?php echo U('Showadmin/addGroup');?>", function(){
		 $('#addGroup').modal();
	});		   
}
</script>
