        ...
        <ul class="nav">
            ...
            <section class="vbox">
                <header class="header bg-white b-b clearfix">
                    <form class="talbe-search" method="get" target="_self" action="<?php echo U('Log/type',array('mid'=>$_GET['mid']));?>">
...
$(function(){
  $("#submit").click(function() {
	...
	$.ajax({
			type: "post",  
			url:"<?php echo U('Log/type',array('mid'=>$_GET['mid']));?>",
			dataType:'json',
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("index.php?g=Qyapp&m=Log&a=typeinfo&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
