...
<!-- 	<div class="wrapper b-b header"><b>流程审批</b></div>
	<ul class="nav">
		<li class="b-b b-light open">
			<a href="<?php echo U('Workflow/index',array('id'=>$_GET['id']));?>"><i class="fa fa-chevron-right pull-right m-t-xs text-xs icon-muted"></i>自定义流程</a>
		</li>				
                    ...
                    <form class="talbe-search" method="get" target="_self" action="<?php echo U('Workflow/index',array('mid'=>$_GET['mid']));?>">
                         <div class="row m-t-sm">
                            <div class="col-sm-8 m-b-xs">
                                <a href="<?php echo U('Workflow/add',array('mid'=>$_GET['mid'],'id'=>$_GET['id']));?>" target="_self" class="btn btn-sm btn-default m-l"><i class="fa fa-plus m-r-xs"></i>新增自定义流程</a>
                            </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover m-b-none entity-view">
								...
								<?php if($data): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($data["id"]); ?>">
                                            ...
                                            <td><?php if($data['status'] == 1): ?>启用<?php endif; if($data['status'] == 0): ?>禁用<?php endif; ?></td>
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("index.php?g=Qyapp&m=Workflow&a=info&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
