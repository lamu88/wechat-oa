...
<body>
	...
	<aside class="aside-md bg-white b-r" id="subNav">
		 ...
		 <div class="wrapper b-b header"><b>离职</b></div>
        <ul class="nav">	
			<li class="b-b b-light">
        	<a href="<?php echo U('Quit/index',array('mid'=>$_GET['mid']));?>" target="_self">
        		<i class="fa fa-chevron-right pull-right m-t-xs text-xs icon-muted"></i>离职查询
        	</a>
        </li>
        <li class="b-b b-light ">
        	<a href="<?php echo U('Quit/conf_man',array('mid'=>$_GET['mid']));?>" target="_self">
            ...
            <section class="vbox">
                ...
                <header class="header bg-white b-b clearfix">
                    <form class="talbe-search" method="post" action="<?php echo U('Quit/index',array('mid'=>$_GET['mid']));?>" target="_self" id="form">
							...
							<div class="col-sm-4 m-b-xs">							
                                <div class="input-group">
                                    ...
                                    <div class="input-group js_show_date js_show_date_0 " id="select_4" >
										<input type="text" class="form-control input-sm" style=" width:230px;" name="username" id="username" value="<?php echo ($username); ?>" placeholder="请输入用户名"/>
										 <input type="text" class="form-control input-sm" style="display:none; width:230px;" name="departname" id="searchDepartname" value="<?php echo ($departname); ?>" placeholder="请输入部门名称、如果多个请用;分开"/>										
                ...
                <section class="scrollable wrapper w-f">
                    <section class="panel panel-default ">
						...
						<?php if($data): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($data["id"]); ?>">
                                        ...
                                        <td><?php echo ($data["bumen"]); ?></td> 
										<td><?php echo ($data["zhiwei"]); ?></td>
										<td><?php echo ($data["quit_time"]); ?></td>
										<td><?php echo ($data["reason"]); ?></td>
										<td><?php if($data["status"] == 0): ?>未审核<?php elseif($data["status"] == 1): ?>通过<?php else: ?>未通过<?php endif; ?></td>
										<td><?php echo (date("Y-m-d H:i:s",$data["time"])); ?></td>
									</tr><?php endforeach; endif; else: echo "" ;endif; ?>	
						<?php else: ?>
									<tr><td colspan='8' class="nodata">暂无信息</td></tr><?php endif; ?>
                                </tbody>
                            </table>
                        </div>
                    </section>
...
$(function(){
  $("#submit").click(function() {
	...
	$.ajax({
			...
			type: "post",  
			url:"<?php echo U('Applyflow/applytype');?>",
			dataType:'json',
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("index.php?g=Qyapp&m=Quit&a=quitInfo&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
