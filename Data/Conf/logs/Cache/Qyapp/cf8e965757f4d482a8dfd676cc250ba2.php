...
<body>
	...
	<div class="wrapper b-b header"><b>售后</b></div>
        <ul class="nav">	
		<!-- <li class="b-b b-light">
        	<a href="<?php echo U('Leave/conf');?>" target="content">
        		<i class="fa fa-chevron-right pull-right m-t-xs text-xs icon-muted"></i>请假查询
        	</a>
		</li> -->
        <li class="b-b b-light ">
        	<a href="<?php echo U('Aftersale/index',array('mid'=>$_GET['mid']));?>" target="_self">
        		<i class="fa fa-chevron-right pull-right m-t-xs text-xs icon-muted"></i>配置审核人
        	</a>
        </li>
        <li class="b-b b-light">
        	<a href="<?php echo U('Aftersale/saleType',array('mid'=>$_GET['mid']));?>" target="_self">
        		<i class="fa fa-chevron-right pull-right m-t-xs text-xs icon-muted"></i>配置售后类型
        	</a>
        </li>
                                    ...
                                    <table class="table table-hover m-b-none entity-view">
                                        ...
                                        <tbody id="mytable">
											<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($data["id"]); ?>">
                                                    ...
                                                    <td><?php if($data['status'] == 1): ?><label class="label label-success">是</label><?php endif; ?>	 			
													    <?php if($data['status'] == 0): ?><label class="label label-default">否</label><?php endif; ?>	
													</td>
...
<form class="form-horizontal form-validate form-modal" target="_self" action="<?php echo U('Aftersale/add',array('mid'=>$_GET['mid']));?>" method="post"> 
...
$(function(){
  $("#submit11").click(function() {
	...
	$.ajax({
			...
			type: "post",  
			url:"<?php echo U('Aftersale/saleType');?>",
			dataType:'json',
			data:'disorder='+order+'&name='+name+'&mid=<?php echo $_GET['mid'];?>&status='+enable,
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("index.php?g=Qyapp&m=Aftersale&a=typeInfo&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
