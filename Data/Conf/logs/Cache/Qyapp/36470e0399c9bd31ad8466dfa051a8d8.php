                ...
                <section class="scrollable  wrapper">
                    <section class="panel panel-default">
                        <div class="panel-body">
                            <form class="form-horizontal form-validate" action="<?php echo U('Performance/index',array('mid'=>$_GET['mid']));?>" method="post">                            
								...
								<div class="js_audit_div" id="autoComplete">
										...
										<div class="form-group" data-num="{{i+1}}">
											...
											<!--	<!--<input type="text" name="username" class="btn btn-default"  id="user" onclick="lookuser(4);" value="<?php echo ($name); ?>"/>
												<select class="form-control js_audit_select" id="user" name="username" onchange="selectAudit($(this))">
												<option value="">请选择</option>
												<?php if(is_array($users)): $i = 0; $__LIST__ = $users;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><option value="<?php echo ($data["id"]); ?>"><?php echo ($data["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
												</select>-->
											</div>
										...
										<!-- <div class="form-group">
											...
											<div class="col-sm-8">
											<?php if(is_array($users)): $i = 0; $__LIST__ = $users;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><label class="checkbox-inline"><input type="checkbox" name="performanceusers[]" data-rule-required="true" value="<?php echo ($data["id"]); ?>"/><?php echo ($data["name"]); ?></label><?php endforeach; endif; else: echo "" ;endif; ?>
											</div>
										</div>-->
										<div class="form-group">
	...
	if(userId==4){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("index.php?g=Qyapp&m=Performance&a=usersInfo&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("index.php?g=Qyapp&m=Borrow_books&a=usersInfo&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
				$('.entity-panel').css('right','0px');				    
			}
		}else{
...
$.ajax({
type:"POST",
url:"index.php?g=Qyapp&m=Borrow_books&a=catDel&mid=<?php echo $_GET['mid'];?>&id="+id,
data:"id="+id,
dataType:'json',
success:function(json){
