...
$(document).ready(function(){ 
	$(".btn-primary").click(function() {
		...
		var subdata='';
		if(btn=="news"){
			subdata='type=mpnews&usertype='+usertype+'&users='+users+'&content='+$('#select_news').val()+'&agentid=<?php echo $_GET['id'];?>&departid='+departid;
		}
		$.ajax({
				type: "post",  
...
<section class="vbox ">
		...
		<header class="header bg-light">
			<ul class="nav nav-tabs nav-white js_message_filetype">
				...
				<li  class="active"><a href="<?php echo U('news_post',array('id'=>$_GET['id']));?>" target="_self">图文</a>
...
<ul class="js_items clearfix" style="position: relative; height: 0px; margin-left:0px;">
			<?php if($list == ''): ?><div class="empty_placeholder_wrap empty_placeholder_graphic ">
						<span class="icon_empty_placeholder"></span>
						<p class="empty_placeholder_text">没有图文素材</p>
				</div><?php endif; ?>
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><li class="tile" data-file-id="<?php echo ($li["id"]); ?>" style="list-style:none; ">
    <div class="video_unit select_<?php echo ($li["id"]); ?>   " ><!-- selected -->
        <p class="video_title"><?php echo ($li["title"]); ?></p>
        <a class="video_image" href="javascript:;">
            <div class="video_image_wrap">
                <img src="<?php echo ($li["pic"]); ?>">
                <span class="image_vertical"></span>
            </div>
        </a>
        <p class="video_detail"><?php echo ($li["desc"]); ?></p>
