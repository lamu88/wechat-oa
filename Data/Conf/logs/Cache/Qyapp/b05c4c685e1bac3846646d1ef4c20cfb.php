                ...
                <form class="talbe-search" method="post"  target="_self"  action="/research/Index/type/0">
                    <div class="row m-t-sm">
                        <div class="col-sm-8 m-b-xs">
                            ...
                            <div class="btn-group">
                                <a href="<?php echo U('Test/add_paper',array('mid'=>$_GET['mid']));?>" target="_self"  class="btn btn-default btn-sm">新增试卷<i class="fa fa-plus m-l-xs m-r-xs"></i></a>
                            </div>
                        </div>
						...
						<table class="table table-hover m-b-none entity-view">
							...
							<TBODY id="mytable">
								<?php if($list): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><TR id="<?php echo ($list["id"]); ?>">
								  ...
								  <TD><a href="<?php echo U('Test/detail',array('id'=>$list['id']));?>" target="_self" ><?php echo ($list["title"]); ?></a></TD> 
								  <TD><?php echo ($list["nums"]); ?></TD> 
								  <TD>
									<?php if($list['type'] == 1): echo ($list["condition"]); ?>分<?php endif; ?>
									<?php if($list['type'] == 2): echo ($list["condition"]); ?>题<?php endif; ?>								  
								  </TD> 	
								  <TD><?php echo ($list["sum"]); ?></TD> 								  
								  
								  <!-- <TD><?php echo (date("Y-m-d H:i",$list["starttime"])); ?></TD>
								  <TD><?php echo (date("Y-m-d H:i",$list["endtime"])); ?></TD>	 -->
								  <TD>
								  <?php if($list['status'] == 1): ?>开启<?php endif; ?>
								  <?php if($list['status'] == 0): ?>关闭<?php endif; ?>	
								  </TD> 	
                                  <TD><?php echo ($list["testtime"]); ?>分钟</TD>								  
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("index.php?g=Qyapp&m=Test&a=paperInfo&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
