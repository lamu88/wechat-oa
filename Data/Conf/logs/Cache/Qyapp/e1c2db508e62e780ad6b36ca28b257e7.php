                    ...
                    <section class="panel panel-default">
                         <div class="table-responsive">
                            <table id="tablelist" class="table table-hover m-b-none entity-view" data-path="/attendance/Rule/RuleDetail?pid={0}">
                              ...
                              <!--  <tbody id="mytable">-->
								<tbody>
								<?php if(is_array($normdata)): $i = 0; $__LIST__ = $normdata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i; ?>
								   <tr>
									<td><?php echo ($data["KPLnorm"]); ?></td>
									<td><?php echo ($data["value"]); ?></td>
									<td><?php echo ($data["gradenorm"]); ?></td>
									<td><a class="btn btn-default"  onclick='edits("<?php echo ($data["id"]); ?>")'>修改</a><!--/<a href="<?php echo U('Performance/normdel',array('id'=>$data['id']));?>">删除</a>--></td>
...
<script type="text/javascript">
	function edits(id){
		$('#apply_edit').load("index.php?g=Qyapp&m=Performance&a=norminfo&mid=<?php echo $_GET['mid'];?>&id="+id, function(){
			 $('#apply_edit').modal();
	...
	// href="<?php echo U('Borrow_books/users_borrows');?>"
		window.location.href="index.php?g=Qyapp&m=Borrow_books&a=users_borrows&mid=<?php echo $_GET['mid'];?>&wecha_id="+wecha_id;
	}
