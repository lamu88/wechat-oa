		...
		<li class="b-b b-light"><a href="<?php echo U('Menu/menu',array('mid'=>$_GET['mid']));?>" target="_self"><i class="fa fa-chevron-right pull-right m-t-xs text-xs icon-muted"></i>自定义菜单</a></li>
            ...
            <section class="vbox">
                ...
                <header class="header bg-white b-b clearfix">
                    <form class="talbe-search" method="post" action="<?php echo U('Card/cardManage',array('mid'=>$_GET['mid']));?>" target="_self" id="form">
                        ...
                        <div class="row m-t-sm">
                            <div class="col-sm-4 m-b-xs">
                                    <a class="btn btn-default btn-sm " data-toggle="reportrange" data-change="submit" id="reportrange">
                                        ...
                                        <input type="hidden" value="2014/11/01 - 2014/11/30" name="reportrange" />
                                    </a>
                            </div>
					...
					<script type="text/javascript">
                        ...
                        <div class="row m-t-sm">					
							...
							function openModel(){
								...
								$('#searchDepartname').attr("value",'');
								$('#myModal').load("/index.php?g=Qyapp&m=Tree&a=myTongji", function(){
									 $('#myModal').modal();
								});	
							}
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover m-b-none entity-view">
								...
								<?php if($data): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($data["id"]); ?>">
										...
										<td><?php echo ($data["userinfo"]["name"]); ?></td>
										<!-- <td><?php echo ($data["wecha_id"]); ?></td> -->
										<td><?php echo ($data["companyinfo"]["公司名称"]); ?></td>
										<td><?php echo ($data["connectinfo"]["手机号码"]); ?></td>									
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("index.php?g=Qyapp&m=Card&a=cardInfo&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
