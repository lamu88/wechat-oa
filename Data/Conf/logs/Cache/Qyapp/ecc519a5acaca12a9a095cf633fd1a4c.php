 ...
 <aside class="aside-md bg-white b-r" id="subNav">
	...
	<ul class="nav">
		<li class="b-b b-light open">
			<a href="<?php echo U('Research/index',array('mid'=>$_GET['mid']));?>" target="_self"><i class="fa fa-chevron-right pull-right m-t-xs text-xs icon-muted"></i>问卷管理</a>
		</li>
                 ...
                 <form class="talbe-search" method="post" action="" target="_self">
                    <div class="row m-t-sm">
                        <div class="col-sm-8 m-b-xs">
                            ...
                            <div class="btn-group">
                                <a href="<?php echo U('Research/add',array('mid'=>$_GET['mid']));?>" target="_self" class="btn btn-default btn-sm">新增问卷信息<i class="fa fa-plus m-l-xs m-r-xs"></i></a>
                            </div>

                            <div class="btn-group m-l">
                                <a href="<?php echo U('Research/index',array('mid'=>$_GET['mid']));?>" target="_self" class="btn btn-sm btn-default " title="全部"><!-- active -->
                                    全部
                                </a>
                                <a href="<?php echo U('Research/index',array('mid'=>$_GET['mid'],'status'=>1));?>" target="_self" class="btn btn-sm btn-default" title="未发布">
                                    未发布
                                </a>
                                <a href="<?php echo U('Research/index',array('mid'=>$_GET['mid'],'status'=>2));?>" target="_self" class="btn btn-sm btn-default" title="已发布">
                                    已发布
                                </a>
                                <a href="<?php echo U('Research/index',array('mid'=>$_GET['mid'],'status'=>3));?>" target="_self" class="btn btn-sm btn-default" title="已暂停">
                                    暂停
                                </a>
                                <a href="<?php echo U('Research/index',array('mid'=>$_GET['mid'],'status'=>4));?>" target="_self" class="btn btn-sm btn-default" title="已结束">
                                    已结束
                                </a>
                            </div>
                        </div>
                ...
                <section class="panel panel-default ">
                    <div class="table-responsive">
                        <table class="table table-hover m-b-none entity-view" data-path="#">
                            ...
                            <tbody id="mytable">
							    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$userlist): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($userlist["id"]); ?>">
                                        <td>
                                            <input type="checkbox" value="">
                                        </td>

                                        <td><?php echo ($userlist["title"]); ?></td>
                                        <td><?php echo ($userlist["type"]); ?></td>
                                        <td><?php echo (date("Y-m-d H:i:s",$userlist["end_time"])); ?></td>
                                        <td>
										<?php if($userlist['status'] == 1): ?>未发布
										<?php elseif($userlist['status'] == 2): ?>已发布
										<?php elseif($userlist['status'] == 3): ?>已暂停
										<?php elseif($userlist['status'] == 4): ?>已结束<?php endif; ?>										
										</td>
                                        <!-- <td><?php echo ($userlist["num"]); ?></td> -->
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("index.php?g=Qyapp&m=Research&a=info&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
