                    ...
                    <form class="talbe-search" method="post" target="_self" action="">
                        <div class="row m-t-sm">
                             <div class="col-sm-3 m-b-xs">
                                <a href="<?php echo U('Attendance/addrule',array('mid'=>$_GET['mid']));?>" target="_self" class="btn btn-default btn-sm"><i class="fa fa-plus m-r-xs"></i>新增规则</a>
                            </div>
                        </div>
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("index.php?g=Qyapp&m=Attendance&a=info&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
