                ...
                <form class="talbe-search" target="_self" method="post" action="">
                    <div class="row m-t-sm">
                        <div class="col-sm-8 m-b-xs">
							...
							<div class="btn-group">
							<a class="btn btn-default btn-sm" href="<?php echo U('Borrow_books/catAdd',array('mid'=>$_GET['mid'],'pid'=>$pid));?>" target="_self">
							添加分类
							</a>
							</div>		
                        </div>
            ...
            <section class="scrollable wrapper w-f">
                <section class="panel panel-default ">
                    <div class="table-responsive">
						...
						<?php if($data['img']): ?><td><img src="<?php echo ($data["img"]); ?>" height="60px" width="60px"></td>
						<?php else: ?>
									<td>暂无分类图片</td><?php endif; ?>
									<td><?php echo ($data["info"]); ?></td>	
									<!--<td><?php echo ($data["title"]); ?></td>-->	
									<td>
									    <?php if($data['status'] == 1): ?>是<?php endif; ?>
			                            <?php if($data['status'] == 0): ?><label><style="color:red;">否<label><?php endif; ?>
									</td>
									<!-- <td>
										<a class="btn default btn-xs green" href="<?php echo U('Borrow_books/cat',array('pid'=>$data['id']));?>">
										    <i class="fa fa-plus"></i>子分类
										</a>							
								 
										<a class="btn default btn-xs purple" href="<?php echo U('Borrow_books/catEdit',array('id'=>$data['id']));?>">
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("index.php?g=Qyapp&m=Borrow_books&a=catInfo&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
...
//删除操作
function catDel(id){
$.post("index.php?g=Qyapp&m=Borrow_books&a=cat_Del&mid=<?php echo $_GET['mid'];?>",
		{'id':id},
		function(d){
			if(d == 1){
				alert("删除成功");
