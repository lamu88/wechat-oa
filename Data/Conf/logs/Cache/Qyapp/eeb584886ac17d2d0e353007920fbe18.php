    ...
    <section class="scrollable  wrapper">
        <section class="panel panel-default">
            <div class="panel-body">
                <form class="form-horizontal form-validate" method="post" target="_self" action="<?php echo U('Card/tpl_set',array('mid'=>$_GET['mid']));?>">
                    <input type="hidden" name="card_id" value=""> 
                     <!--<input type="hidden" id="checkSelect" name="checkSelect" value="0"> -->
                    <div class="form-group">
					    <div class="col-sm-7">
                                    <label class="checkbox-block" data-toggle="refresh">
                                     <b class="" style="font-size:16px">选择模板</b>
                                    </label>											
                        </div> 
                    </div>	
					<div class="form-group" id="cardList">
						<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><article id="<?php echo ($data["id"]); ?>" class="post-83 post type-post status-publish format-standard has-post-thumbnail hentry category-featured category-tutorials col-md-4 col-sm-4 pbox">
							<input type="radio" name="temp" <?php if($data['id'] == $info['muban']): ?>checked<?php endif; ?> > <?php echo ($data["name"]); ?>
							<br>							
							<a href="javascript:void();" target="_self"><img class="img-responsive" src="<?php echo ($data["img"]); ?>" width="396px" height="217px"></a>
							<br>							
					...
					<button class="btn btn-primary" type="submit" data-loading-text="保存中..."> 保存模板</button>
                ...
                <input type="hidden" name="__hash__" value="d87610055142ec516de696412ff56cad_50d2603d6fd75e4101a575536355c2ac"></form>
            </div>
        </section>
    </section>
</section>
...
$('#cardList article').each(function(){
    $(this).click(function(){
		var id = $(this).attr('id');
		$('input[name=card_id]').attr('value',id);
	});
});
function selectCard(obj){
    //alert(obj.find('input').attr('type'));
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				$('#info').load("./index.php?g=Qyapp&m=Hiring&a=info&id="+userId);
				$('.entity-panel').removeClass('hd');
