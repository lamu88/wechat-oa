 	 ...
 	 <aside>
            <section class="vbox">
                <header class="header bg-white b-b clearfix">
                    <form class="talbe-search" method="get" action="<?php echo U('Hiring/position',array('mid'=>$_GET['mid']));?>" target="_self">
                        <div class="row m-t-sm">
                            <div class="col-sm-9 m-b-xs">                       
                                <a href="<?php echo U('Hiring/addPosition',array('mid'=>$_GET['mid']));?>" target="_self" class="btn btn-sm btn-default"><i class="fa fa-plus m-r-xs"></i>发布新职位</a>
                                <div class="btn-group m-l">
                                    <a href="<?php echo U('Hiring/position',array('mid'=>$_GET['mid']));?>"  target="_self"class="btn btn-sm btn-default <?php if(!$status): ?>active<?php endif; ?>" title="全部">
                                        全部
                                    </a>
                                    <a href="<?php echo U('Hiring/position',array('status'=>1,'mid'=>$_GET['mid']));?>" target="_self" class="btn btn-sm btn-default <?php if($status == 1): ?>active<?php endif; ?>" title="发布中">
                                        发布中
                                    </a>
                                    <a href="<?php echo U('Hiring/position',array('status'=>2,'mid'=>$_GET['mid']));?>" target="_self" class="btn btn-sm btn-default <?php if($status == 2): ?>active<?php endif; ?>" title="未发布">
                                        未发布
                                    </a>
                                    <a href="<?php echo U('Hiring/position',array('status'=>3,'mid'=>$_GET['mid']));?>" target="_self" class="btn btn-sm btn-default <?php if($status == 3): ?>active<?php endif; ?>" title="已暂停">
                                        已暂停
                                    </a>
                                    <a href="<?php echo U('Hiring/position',array('status'=>4,'mid'=>$_GET['mid']));?>" target="_self" target="_self" class="btn btn-sm btn-default <?php if($status == 4): ?>active<?php endif; ?>" title="已结束">
                                        已结束
                                    </a>
                            ...
                            <table class="table table-hover m-b-none entity-view">
                                <thead>
                                    <tr>
                                        ...
                                        <th>职位名称</th>
                                        <th>所属部门</th>
                                        <th class="th-sortable" data-sort-name="hiring_position_rate">点击量</th>
                                        <th class="th-sortable" data-sort-name="hiring_position_resume">投递简历数</th>
                                        <th class="th-sortable" data-sort-name="hiring_position_employ">录用数</th>
                                        <th>状态</th>
                                        <th>发布时间</th>
                                        <th class="th-sortable" data-sort-name="hiring_position_end_time">结束时间</th>
                                        <th>排序序号</th>
                                    </tr>
                                </thead>
								<tbody id="mytable">
								<?php if($data): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($data["id"]); ?>">
										...
										<td><?php echo ($data["resume_num"]); ?></td>
										<td><?php echo ($data["num"]); ?></td>										
										<td>
											<?php if($data['status'] == 0): ?>未发布<?php endif; ?>										
											<?php if($data['status'] == 1): ?>发布中<?php endif; ?>
											<?php if($data['status'] == 3): ?>已暂停<?php endif; ?>
											<?php if($data['status'] == 2): ?>已结束<?php endif; ?>
										</td>
										<td><?php echo (date("Y-m-d H:i:s",$data["create_time"])); ?></td>	
										<td><?php echo (date("Y-m-d H:i:s",$data["end_time"])); ?></td>
										<td><?php echo ($data["disorder"]); ?></td>										
									</tr><?php endforeach; endif; else: echo "" ;endif; ?>					
								<?php else: ?>
								<tr><td colspan='10' class="nodata">暂无职位</td></tr><?php endif; ?>
								</tbody>														
                            </table>
                        </div>
                    </section>
	...
	$(this).each(function(){
		...
		if($('#user'+userId).length>0){
		    ...
		    if(curId == 'user'+userId){
				...
				$('#info').empty();   
				//并ajax加载点击行的数据信息
				$('#info').load("index.php?g=Qyapp&m=Hiring&a=positionInfo&mid=<?php echo $_GET['mid'];?>&id="+userId);
				$('.entity-panel').removeClass('hd');
