...
class Think {
    ...
    static private function buildApp() {
        // 加载底层惯例配置文件
        C(include THINK_PATH.'Data/Conf/convention.php');
        ...
        // 加载框架底层语言包
        L(include THINK_PATH.'Lanrain/Lang/'.strtolower(C('DEFAULT_LANG')).'.php');
        ...
        if(C('APP_TAGS_ON')) {
            if(isset($mode['extends'])) {
                ...
                C('extends', include THINK_PATH.'Data/Conf/tags.php');
            }
        }
        ...
        if(APP_DEBUG) {
            // 调试模式加载系统默认的配置文件
            C(include THINK_PATH.'Data/Conf/debug.php');
            // 读取调试模式的应用状态
            $status  =  C('APP_STATUS');
            // 加载对应的项目配置文件
