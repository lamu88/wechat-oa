...
var tmr_slogan;
	/*查询参与人数*/
	 function getConnectNum(){
	 	$.ajax({
	 		type: "post",
	 		url : "/index.php?g=Qyapp&m=Scene&a=getConnectNum",
	 		dataType:'text',
...
function cutdown_start() {
	...
	tmr_cutdown_start = window.setInterval(function() {
		b--;
		if (b == 0){
			$.getJSON("/index.php?g=Qyapp&m=Scene&a=startShake", {
				id: id		
			},function(c) {
				if (c.err == 0) {
					a.html("GO!");
...
/*获取用户数据*/
function gameRun(){
	$.getJSON("/index.php?g=Qyapp&m=Scene&a=shakeRun", {
			...
			if (d.status == 0) {
				...
				if(d.res){
					...
					for (var i = 0; i < d.res.length; i++) {
						temp += '<div class="player" uid="'+d.res[i].id+'" style="top: '+size*i+'px;left:'+d.res[i].mLeft+';"><div class="head shake" style="height:'+size+'px;width:'+size+'px;background-image: url('+d.res[i].portrait+'); border-radius: 100% !important;"></div><div class="nickname" style="line-height:'+size+'px;height:'+size+'px;">'+d.res[i].nickname+'</div></div>';
					};
				}
