$('.begin').click(function(){
    ...
    if ($(this).hasClass("disable")) {
        ...
        $.getJSON("/index.php?g=Apps&m=Scene&a=get_vote", {'vote_id':vote_id,'scene_id':scene_id}, function(data){
            if (data.err == 0) {
                $(".Panel.VoteList").css({
                    opacity: 0,
                    display: "none"
    ...
    function getVcount(){
        $.getJSON("/index.php?g=Apps&m=Scene&a=vote_count", {'vote_id':vote_id}, function(data){
            $('.fz').html(data.count);
            $('.fm').html(data.fcount);
        });
    ...
    $('.inf-btn').click(function(){
        if (!$(this).hasClass("begining") && !$(this).hasClass("endgame")) {
            $.getJSON("/index.php?g=Apps&m=Scene&a=vote_start", {'vote_id':vote_id}, function(data){
                if(data.err==0){
            ...
            $.getJSON("/index.php?g=Apps&m=Scene&a=vote_stop", {'vote_id':vote_id}, function(data){
    ...
    function showVote(){
            $.getJSON("/index.php?g=Apps&m=Scene&a=ajaxVcount", {'vote_id':vote_id}, function(data){
                if(data.flag == 1){
