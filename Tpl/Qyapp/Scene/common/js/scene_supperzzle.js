...
function addLog(nid,vid){
    $.post("/index.php?g=Apps&m=Scene&a=add_slog", {'sceneid':sceneid,'nid':nid,'vid':vid}, function(f){});
}
...
function defData(){
     $.getJSON("/index.php?g=Apps&m=Scene&a=defApps", {'sceneid':sceneid}, function(f){
        if(f.err == 0  && f.data){
                    if (f.data.list) {
                        var g = f.data.list;
