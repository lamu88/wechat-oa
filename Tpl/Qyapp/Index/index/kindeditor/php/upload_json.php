...
$php_path=substr($php_path, 0,strlen($php_path)).'/../AppStore/';
$php_url = dirname($_SERVER['PHP_SELF']) . '/';
$save_path=$php_path;
$save_url = 'http://'.$_SERVER["HTTP_HOST"].'/AppStore/';//访问时的网络地址
$ext_arr = array(
	'image' => array('gif', 'jpg', 'jpeg', 'png', 'bmp','pdf'),	
	'file' => array('zip'),
);
//D:/wamp/www/appopen.ws/Public/kinded/../Uploads/
$max_size = 100000000;
...
if (empty($_FILES) === false) 
{
	...
	$m= date("m");
	$d= date("d");
	$save_path .= $y . "/".$m."/".$d."/".$_GET['name'].'/'.$_GET['appname'].'/'.$_GET['version'].'/';
	$save_url .= $y . "/".$m."/".$d."/".$_GET['name'].'/'.$_GET['appname'].'/'.$_GET['version'].'/';
	...
	echo $json->encode(array('error' => 0,'url' => $file_url,'file_path'=>$file_path));
	exit;
}
function alert($msg) 
{
