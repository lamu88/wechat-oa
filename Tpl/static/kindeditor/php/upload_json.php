...
$save_path = $php_path . '../../../../Tpl/Uploads/';//上传的磁盘地址
...
if (empty($_FILES) === false) {
	...
	if(isset($_GET["isadd"])&&$_GET["isadd"]==1)
	{
		...
		$data["type"]=$file_ext;
		$data["time"]=time();
		$furl='http://'.$_SERVER["HTTP_HOST"]."/index.php?g=User&m=Attachment&a=fileAdd&token=".$data["token"]."&url=".$data["url"]."&size=".$data["size"]."&type=".$data["type"]."&time=".time();
		file_get_contents($furl);		
		//header('Content-type: text/html; charset=UTF-8');
		//header("Locatoin:?code=200&url=".$file_url);
		//exit;
