...
class UsersAction extends BaseAction{
	...
	public function checklogin(){
		...
		if($res&&($pwd===$res['password'])){
			...
			$this->success('登录成功',U('Apps/Index/index'));
		}else{
		...
		if($db->create()){
			...
			if($id){				
				if(C('ischeckuser')!='true'){
					$this->success('注册成功,请联系在线客服审核帐号',U('Apps/Index/index'));exit;
				}
				...
				$this->success('注册成功',U('Apps/User/index'));
			}else{
				$this->error('注册失败',U('Index/index'));
			}
		}else{
			$this->error($db->getError(),U('Index/index'));
		}
	}
	...
	public function checkpwd(){
		...
		if($list==false) $this->error('邮箱和帐号不正确',U('Index/index'));
