...
class WeixinAction extends BaseAction{
	...
	public function __construct(){
		...
		if (!$this->token){
			//
			$product_cart_model=M('Store_cart');
		...
		//dump($this->wecha_id);die;
		//读取微信支付配置
		$payConfig = M('alipay_config')->where(array('token'=>$this->token))->find();
		$payConfigInfo = json_decode($payConfig['info'],true);
...
/*
	public function index(){
		...
		//使用jsapi接口
		// $conf= new Common_util_pub();
		$appId = 'wxc7b85bf1041a1cfe';
		$mchid = '10030219';
		$key = 'mmx9987dreamgoodluck8857aiss1984';
		$appsecret = '63b4d226fcaeeb2f89a796f40a9dfa11';
		$jsApi = new JsApi_pub($appId,$mchid,$key,$appsecret);
		$openid = $_GET['wecha_id'];
		$unifiedOrder = new UnifiedOrder_pub($appId,$mchid,$key,$appsecret);
		...
		$unifiedOrder->setParameter("notify_url",'http://xingky.wxopen.cn/wxpay/notify_url.php');//通知地址 
...
/**-----------------------------------------参考结束-----------------------------------------------*/		
	public function new_pay(){
		...
		if (!isset($_GET['code'])){
			//触发微信返回code码
			$url = $jsApi->createOauthUrlForCode(urlencode('http://xingky.wxopen.cn/wxpay/index.php?g=Qyapp&m=Weixin&a=new_pay&price='.$_GET['price'].'&orderName='.urlencode($_GET['orderName']).'&single_orderid='.$_GET['single_orderid'].'&showwxpaytitle=1&from='.$_GET['from'].'&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id']));
			//print_r($url);exit;
			Header("Location: $url"); 
		}
		...
		*/
		...
		$unifiedOrder->setParameter("notify_url",'../wxpay/notify_url.php');//通知地址 
		$unifiedOrder->setParameter("trade_type","JSAPI");//交易类型
	...
	public function pay(){
		...
		$wxPayHelper->setParameter("notify_url", $this->siteUrl.'/index.php?g=Qyapp&m=Weixin&a=return_url&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&from='.$_GET['from']);
		...
		$returnUrl='/index.php?g=Qyapp&m='.$from.'&a=payReturn&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&orderid='.$orderid;
		$this->assign('returnUrl',$returnUrl);
