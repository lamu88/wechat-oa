...
class BaseAction extends Action
{
    ...
    protected function all_insert($name = '', $back = '/index')
    {
        ...
        if ($db->create() === false) {
            ...
            if ($id) {
                ...
                $this->success('操作成功', U(MODULE_NAME . $back,array('token'=>$_SESSION['token'])));
            } else {
                $this->error('操作失败', U(MODULE_NAME . $back,array('token'=>$_SESSION['token'])));
            }
        }
    }
        ...
        if ($db->create() === false) {
            ...
            if ($id) {
                ...
                if (in_array($name, $m_arr)) {
                    ...
                    $da['type'] = $this->_post('type','intval',1);
    ...
    protected function all_del($id, $name = '', $back = '/index')
    {
        ...
        $db   = D($name);
        if ($db->delete($id)) {
            $this->ajaxReturn('操作成功', U(MODULE_NAME . $back,array('token'=>$_SESSION['token'])));
        } else {
            $this->ajaxReturn('操作失败', U(MODULE_NAME . $back,array('token'=>$_SESSION['token'])));
        }
    }
